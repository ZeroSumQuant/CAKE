...
class RuleCreator:
        ...
        for error_type, pattern_config in self.pattern_library.items():
            ...
            if match:
                ...
                if pattern_config["fix_template"] and match.groups():
                    try:
                        ...
                        fix_cmd = pattern_config["fix_template"].format(
                            module=match.group(1), file=match.group(1)
                        )
                ...
                proposal = RuleProposal(
                    ...
                    test_cases=[{"stage": stage, "error": error, "should_match": True}],
                )
    ...
    async def _enhance_with_claude(
        self, proposal: RuleProposal, context: Dict[str, Any]
    ) -> Optional[RuleProposal]:
    ...
    def _parse_claude_response(
        self, content: str, stage: str, error: str
    ) -> Optional[RuleProposal]:
    ...
    def _generate_cache_key(self, stage: str, error: str) -> str:
        """Generate cache key for proposal."""  # Normalize error for caching
    ...
    def _estimate_time_saved(self, proposal: RuleProposal) -> float:
        """Estimate time saved by automation (seconds)."""  # Base estimates by command type
    ...
    def _estimate_prevented_failures(self, proposal: RuleProposal) -> int:
        """Estimate number of future failures prevented."""  # Based on confidence and pattern type
    ...
    def test_proposal(self, proposal: RuleProposal) -> Dict[str, Any]:
        ...
        for test_case in proposal.test_cases:
            try:
                ...
                context = {"stage": test_case["stage"], "error": test_case["error"]}
...
if __name__ == "__main__":
    ...
    class MockClaudeClient:
        async def chat(self, prompt: str, max_tokens: int = 800):
            ...
            if "ModuleNotFoundError" in prompt:
                ...
                content = """SIGNATURE: validate:PermissionError:script
    ...
    async def test_generation():
        ...
        if proposal2:
            ...
            print(
                f"  Tests: {test_results['passed']}/{test_results['passed'] + test_results['failed']} passed"
            )
