...
class TaskRun(SQLModel, table=True):
    ...
    constitution_id: UUID = Field(foreign_key="constitutions.id")
...
class StageExecution(SQLModel, table=True):
    ...
    __table_args__ = (
        Index("ix_stage_task", "task_run_id", "stage_name"),
    )
    
    id: UUID = Field(default_factory=uuid4, primary_key=True)
    task_run_id: UUID = Field(foreign_key="task_runs.id", index=True)
...
class StrategicDecision(SQLModel, table=True):
    ...
    __table_args__ = (
        Index("ix_decision_task_time", "task_run_id", "timestamp"),
    )
...
class AutomationRule(SQLModel, table=True):
    ...
    # Audit trail
    created_at: datetime = Field(default_factory=datetime.utcnow)
    created_by_task_id: Optional[UUID] = Field(foreign_key="task_runs.id")
...
class RuleApplication(SQLModel, table=True):
    ...
    stage_execution_id: UUID = Field(foreign_key="stage_executions.id")
...
class ErrorPattern(SQLModel, table=True):
    ...
    __table_args__ = (
        Index("ix_pattern_hash_stage", "pattern_hash", "stage"),
    )
...
class KnowledgeEntry(SQLModel, table=True):
    ...
    __table_args__ = (
        Index("ix_knowledge_type_tags", "entry_type", "tags"),
    )
    ...
    related_entries: List["KnowledgeEntry"] = Relationship(
        back_populates="related_entries",
...
class KnowledgeRelation(SQLModel, table=True):
    ...
    source_id: UUID = Field(foreign_key="knowledge_entries.id", primary_key=True)
    target_id: UUID = Field(foreign_key="knowledge_entries.id", primary_key=True)
...
class PerformanceMetric(SQLModel, table=True):
    ...
    __table_args__ = (
        Index("ix_metric_time", "metric_name", "timestamp"),
    )
    ...
    # Context
    task_run_id: Optional[UUID] = Field(foreign_key="task_runs.id", index=True)
    metadata: Dict[str, Any] = Field(default_factory=dict, sa_column=Column(JSON))
