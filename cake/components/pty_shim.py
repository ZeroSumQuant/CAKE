...
class PTYShim:
    ...
    DEFAULT_ALLOWED = [
        ...
        # Testing
        'pytest', 'python -m pytest', 'unittest', 'nose', 'tox',
        ...
        'echo', 'touch', 'mkdir', 'cp', 'mv',
        
        # Development tools
        'make', 'docker build', 'docker run', 'npm', 'yarn',
        'cargo', 'go', 'node',
    ...
    def _filter_input(self, data: bytes, command_buffer: str) -> Optional[bytes]:
        """Filter input data for safety."""
        # Check if this completes a command (newline)
...
class PTYShimIntegration:
    ...
    def __init__(self, cake_adapter):
        ...
        self.pty_shim = PTYShim(
            intervention_callback=self._check_command_intervention
        )
    
    def _check_command_intervention(self, command: str) -> Optional[str]:
        """Check if CAKE wants to intervene on this command."""
        # Update CAKE state
        action = {
...
if __name__ == "__main__":
    import tempfile
    from cake_adapter import create_cake_system
