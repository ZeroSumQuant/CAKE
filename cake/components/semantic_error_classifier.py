...
class ErrorPatternExtractor:
    ...
    def __init__(self):
        ...
        self.semantic_vectorizer = CountVectorizer(
            max_features=500, token_pattern=r"\b[a-zA-Z_][a-zA-Z0-9_]*\b"
        )
    ...
    def _extract_error_type(self, error_message: str) -> str:
        """Extract the primary error type."""  # Look for Python exception types
    ...
    def _extract_semantic_tokens(self, error_message: str) -> List[str]:
        """Extract semantically meaningful tokens."""  # Technical terms pattern
    ...
    def _extract_actionable_elements(self, error_message: str) -> List[Dict[str, Any]]:
        ...
        file_matches = re.findall(
            r"create.*?file.*?['\"]([^'\"]+)['\"]", error_message, re.IGNORECASE
        )
    ...
    def _basic_similarity(self, error1: str, error2: str) -> float:
        """Basic similarity calculation when semantic models unavailable."""  # Simple token overlap
...
class ErrorSignatureDatabase:
    ...
    def find_matching_signatures(
        self, features: Dict[str, Any], max_results: int = 5
    ) -> List[Tuple[ErrorSignature, float]]:
    ...
    def _calculate_signature_similarity(
        self, features: Dict[str, Any], signature: ErrorSignature
    ) -> float:
        ...
        tag_overlap = len(feature_tags & signature.semantic_tags) / max(
            len(signature.semantic_tags), 1
        )
        similarity += tag_overlap * 0.3

        # Pattern match (20% weight)
        feature_patterns = set(p["pattern_name"] for p in features["matched_patterns"])
        pattern_overlap = len(feature_patterns & set(signature.context_patterns)) / max(
            len(signature.context_patterns), 1
        )
    ...
    def _update_index(self, signature: ErrorSignature):
        """Update search index for signature."""  # Index by error type
...
class SemanticErrorClassifier:
    ...
    def __init__(self, persistence_path: Path):
        ...
        logger.info(
            f"SemanticErrorClassifier initialized with {len(self.signature_db.signatures)} signatures"
        )
    ...
    def classify_error(
        ...
        cache_key = hashlib.md5(
            f"{error_message}_{str(context)}".encode(), usedforsecurity=False
        ).hexdigest()
        ...
        logger.info(
            f"Classified error as {best_signature.error_type} with {confidence:.2f} confidence"
        )
    ...
    def _infer_category_and_severity(
    ...
    def _generate_context_insights(
        ...
        for stage in context_clues.get("stage_hints", []):
            insights["stage_relevance"][
                stage
            ] = f"Error patterns suggest this occurred during {stage} stage"
        ...
        if 0 <= current_hour <= 6:
            insights["timing_factors"][
                "late_night"
            ] = "Late night coding - check for fatigue-related errors"
        elif 17 <= current_hour <= 23:
            insights["timing_factors"][
                "evening"
            ] = "Evening work session - consider environment setup issues"
    ...
    def learn_from_resolution(
        ...
        """Learn from error resolution outcomes."""  # Classify the error to get signature
        ...
        logger.info(
            f"Learned from resolution: {resolution_method} -> {'success' if resolution_success else 'failure'}"
        )
    ...
    def _get_resolution_effectiveness(self) -> Dict[str, Any]:
        """Get effectiveness metrics for different resolution approaches."""  # This would require tracking resolution outcomes
