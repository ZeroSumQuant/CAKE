#!/usr/bin/env python3
"""snapshot_manager.py - Automatic snapshot and rollback for CAKE
...
class SnapshotManager:
    ...
    def create_snapshot(
        ...
        if dirty_files:
            stash_message = f"CAKE_SNAPSHOT_{snapshot_id}: {description}"
            subprocess.run(
    ...
    def rollback_to_snapshot(self, snapshot_id: str, keep_changes: bool = True) -> bool:
        ...
        try:
                ...
                if current_dirty:
                    subprocess.run(
                        [
                            ...
                            "push",
                            "-m",
                            f"CAKE_ROLLBACK_PRESERVE_{datetime.now().isoformat()}",
                        ],
                        cwd=self.repo_path,
                        check=True,
                    )
            ...
            stash_pattern = f"CAKE_SNAPSHOT_{snapshot_id}"
    ...
    def _load_snapshot_index(self):
        ...
        try:
            result = subprocess.run(
                ["git", "config", "--local", "--get", "cake.snapshots"],
                cwd=self.repo_path,
                capture_output=True,
                text=True,
                check=True,
...
class SnapshotIntegration:
    """Integration between snapshot manager and CAKE."""

    def __init__(self, cake_adapter, repo_path: Path = Path.cwd()):
        """Initialize snapshot integration."""
        self.cake_adapter = cake_adapter
        self.snapshot_manager = SnapshotManager(repo_path)

        # Hook into CAKE for auto-snapshots
        self.cake_adapter.add_pre_message_hook(self._pre_intervention_snapshot)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="CAKE Snapshot Manager")
    ...
    cleanup_parser.add_argument(
        "--days", type=int, default=7, help="Keep snapshots newer than N days"
    )
    ...
    try:
        ...
        if args.command == "create":
            ...
            for s in snapshots:
                print(
                    f"{s.snapshot_id} | {s.timestamp.strftime('%Y-%m-%d %H:%M')} | {s.description}"
                )
