...
class SyntaxErrorFixer:
            ...
            if '"""' in problem_line:
                ...
                if len(quote_positions) >= 2 and len(quote_positions) % 2 == 0:
                    ...
                    if after_quotes.strip():
                        ...
                        self.log(f"Fixed: {filepath}:{line_num} - moved code to next line", "SUCCESS")
                        return True
                        
            # Pattern 2: Unterminated string literals
            if "unterminated string literal" in lines[idx]:
                # This is often a comment, we'll skip
                self.log(f"Skipping unterminated string in comment: {filepath}:{line_num}", "WARNING")
                return False
    ...
    def fix_indentation_error(self, filepath: str, line_num: int) -> bool:
        ...
        try:
            ...
            self.log(f"Error fixing indentation in {filepath}:{line_num} - {str(e)}", "ERROR")
            return False
...
def main():
    ...
    parser.add_argument(
        '--check', '-c',
        action='store_true',
        help='Check only, do not fix (dry run)'
    )
